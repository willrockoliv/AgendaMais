...
namespace AgendaMais.Classes.DAOs
{
    class GrupoProdutoDAO : DAO
    {
        ...
        public static List<GrupoProdutoVO> GetTodosRegistrosAtivos()
        {
            return MontaListVO(DAO.GetTodosRegistrosAtivos("grupo_produto"));
        }

        public static List<GrupoProdutoVO> GetTodosRegistrosNaoAtivos()
        {
            return MontaListVO(DAO.GetTodosRegistrosNaoAtivos("grupo_produto"));
        }
        ...
        public static void DeletarRegistro(int id)
        {
            ...
            List<string> sql = new List<string>();
            sql.Add($"UPDATE grupo_produto SET ativo={false} WHERE id={id}");
            sql.Add($"UPDATE produto SET ativo={false} WHERE id_grupo_produto={id}");
            ExecutaSQL(sql);
        }
    }
}
